...
class MoveBaseServiceTest {
    ...
    void can_select_by_id() {
        when(mockMoveBaseDAO.getMoveBaseById(eq(MOVE_BASE_ID1))).thenReturn(moveBase1);

        MoveBase actual = moveBaseService.getMoveBaseById(MOVE_BASE_ID1);
    ...
    void can_select_by_type() {
        when(mockMoveBaseDAO.getMoveBaseByType(eq(PokemonType.ELECTRIC.name()))).thenReturn(Arrays.asList(moveBase2, moveBase3));

        List<MoveBase> actual = moveBaseService.getMoveBaseByType(PokemonType.ELECTRIC.name());
